...
twoPersons :: Applicative f =>
  ...
  -> f [Person]
twoPersons n1 a1 e1 n2 a2 e2 = liftA2 (:) (Person <$> n1 <*> a1 <*> e1) (pure <$> Person <$> n2 <*> a2 <*> e2)

-- The above can be simplified for clarity:
      ...
      let parseOp = case op of
          ...
          parseArg str =
            ...
            in case (isVar, isNum, readMaybe str :: Maybe Int) of
              ...
              (True, True, _) -> pure (Variable (head str)) -- covers both? just Variable prevails
          a1 = parseArg arg1
          a2 = parseArg arg2
      in parseOp <*> a1 <*> a2
...
myFmap f x = myLiftA2 (\_ b -> f b) (myPure ()) x
