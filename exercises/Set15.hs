...
-- Convert String error to [T.Text] to please invalid
toErr :: String -> [T.Text]
toErr = pure . T.pack
...
validateDiv _ 0 = invalid (toErr "Division by zero!")
...
validateAddress streetName streetNumber postCode =
  ...
  where
    checkStreet s =
      if length s <= 20 then pure s else invalid (toErr "Invalid street name")
    checkNumber s =
      if all isDigit s then pure s else invalid (toErr "Invalid street number")
    checkPostcode s =
      if length s == 5 && all isDigit s then pure s else invalid (toErr "Invalid postcode")
...
boolOrInt s =
  let b = case map toLower s of
            ...
            _ -> invalid (toErr "Not a Bool")
      i = case readMaybe s of
            Just n -> pure (Right n)
            Nothing -> invalid (toErr "Not an Int")
  in b <|> i
...
normalizePhone s =
  let stripped = filter (/= ' ') s
      tooLong = if length stripped > 10 then invalid (toErr "Too long") else pure ()
      checkChars [] errs acc =
        if null errs then pure (reverse acc)
        else invalid (map (T.pack . (\c -> "Invalid character: " ++ [c])) errs)
  ...
  case words s of
    [arg1, op, arg2] ->
      let parseOp = case op of
            "+" -> pure Plus
            "-" -> pure Minus
            _ -> invalid (toErr ("Unknown operator: " ++ op))
          parseArg str =
            ...
            in case (isVar, isNum, readMaybe str :: Maybe Int) of
              ...
              (False, True, Nothing) -> invalid (toErr ("Invalid number: " ++ str) ++ toErr ("Invalid variable: " ++ str))
              (False, False, _) -> invalid (toErr ("Invalid number: " ++ str) ++ toErr ("Invalid variable: " ++ str))
              (True, True, _) -> pure (Variable (head str))
          a1 = parseArg arg1
          a2 = parseArg arg2
      in parseOp <*> a1 <*> a2
    _ -> invalid (toErr ("Invalid expression: " ++ s))
