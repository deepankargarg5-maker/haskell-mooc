...
-- Helper to join multiple error messages into one string
...
validateAddress streetName streetNumber postCode =
  Address <$> checkStreet streetName <*> checkNumber streetNumber <*> checkPostcode postCode
  where
    checkStreet s =
...
-- Ex 10: Define Both and its instances

-- This fixes the test errors (missing Both type)
